...
- Включить дефолтное контекстное меню сцены:
  
  ```python
  view.enable_default_context_menu()
  ```
  ...
  ```python
  from boardview.utils import install_ru_translator
...
- Повернуть на 90° по часовой стрелке вставленные элементы. Для этого нужно вставить элементы и вызвать контекстное меню, нажав **ПРАВУЮ** кнопку мышки (при этом должно быть включено дефолтное контекстное меню сцены). Либо можно вставить элемент и нажать **Пробел**.
...
- Рисовать точку с помощью контекстного меню (при этом должно быть включено дефолтное контекстное меню сцены) и кликом **ПРАВОЙ** кнопки мышки с нажатой клавишей **Shift**. **ВАЖНО**: точку можно нарисовать только в пределах границ элемента.
